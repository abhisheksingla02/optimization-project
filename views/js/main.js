...
// rePizzas(size) is called when the slider in the "Our Pizzas" section of the website moves.
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    if (size == 1) {
      ...
      newwidth = 50;
    } else console.log("bug in size switcher");
    //to avoid forced synchronous layout the variable is declared outside the loop.
...
function updatePositions() {
  ...
  // document.body.scrollTop is saved in new variable so that when the loop runs every time dom is not called again and again.
  var ac = document.body.scrollTop / 1250;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((ac) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //iterations have been reduced according to height of screen and number of rows present.
  for (var i = 0; i < j; i++) {
